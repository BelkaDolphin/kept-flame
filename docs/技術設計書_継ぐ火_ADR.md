...
- **(a) saveSchemaVersion 差** → 純関数マイグレーション version 順連鎖。**[2026-07-30追記]** payload 軸の最初の実段 = M16 の v1→v2（facility footprint 追加。省略可フィールドでも「旧ビルドが新セーブを読むと盤面幾何が黙って潰れる」類の変更は版差で塞ぐ、が線引きの実例）。v2→v3 = M21（派遣スナップショット追加。旧ビルドは未帰還派遣を黙って捨て派遣中住民が永久拘束されるため同じ線引きで bump）。**[2026-07-31追記]** v3→v4 = M22（派遣スナップショットへの event 効果 `effects`/choice・branch 記録の追加。旧ビルドは効果を黙って落とし「燃えるはずの記録が燃えない」ため同じ線引きで bump）。v4→v5 = M52（地形 `terrain{rubbleCells, reclaimedCount}` 追加。旧ビルドの往復で瓦礫配置と解放数が不可逆に消え、開墾コスト指数 `1.15^n` がリセットされる経済 exploit が成立するため bump。判断の両論は `migration.ts` の `migratePayloadV4ToV5` doc に記録）。**[2026-08-01追記]** v5→v6 = M28（周回データ `progression{runCount, cumulativeInheritPoints, purchasedTiers}` 追加。旧ビルドの往復で ①累計継承点・購入段数が不可逆に消える ②次周シード = `hash(前seed, 周回回数, 累計継承点)` のため**同じセーブから別の世界が導出される決定論分岐** ③購入段数消失で上限クランプ(GDD 11.4-6)を再購入で破れる、の3点により bump。両論は `migratePayloadV5ToV6` doc に記録）。**[2026-08-01裁定・台帳v7 必-1]** 周回データのセーブ内キー配置は、本 ADR 旧文が示唆したトップレベル個別キー(`runCount` 等の列挙)ではなく、**省略可オブジェクト `progression` への集約が正**（購入段数という第3の値が必須になったこと、必須トップレベルキーの追加は golden 全ベクタのバイト列を動かすことが理由。省略時の既定値 = 1周目・0点・全系統0段）。**[2026-08-02裁定・台帳v13 必-2]** v6→v7 = プレイテスト R2-A01 修正（欠落していた廃材 `waste` の resource entity を在庫0で補充する**データ修復(repair)段**）。ここで migration の第2類型を正本化する: **データ修復(repair)段は、旧ビルド破壊の有無に関わらず版境界を必要とする**（連鎖は版でしか段を選べず、版差が救済の運搬手段そのものであるため。従来5段の「旧ビルドが新セーブを読むと黙って壊れる」線引きとは独立の bump 根拠）。運用規約: **修復に使う資源ID等は修復時点の content 宣言値をコードに歴史的定数として凍結**し、content 側が将来変わっても追随しない（migration の純関数性維持。変えるなら新しい版を切る）。復元値はセーブに記録済みの観測値のみ（在庫0復元。`cumulativeOverflow` からの逆算値のような再計算値を migration が発明しない）。両論は `migratePayloadV6ToV7` doc に記録。
