...
| engine コマンド(`src/engine/commands.ts` の `apply(Command)`) | 未割当(M16〜M19/M30 の周辺) | **未実装**。§4 の `stateApplied` が暫定の口 |
...
テスト: `tests/ui/{reactive,store,derived,screens}.test.ts` + `fixtures.ts`(**69 件**)。**fan-in 上界の検収は `tests/ui/derived.test.ts` の先頭 describe** が本体。
...
| `stateApplied{state, reason}` | **暫定**: コマンド適用(§7-2) | 同じ世界の次の state を据える | worldSeed / algoVersion 相違、過去 tick |
| `cellSelected{cellIndex}` | ②のタップ(M18/M30) | 選択セル更新 | 0〜47 の範囲外 |
| `screenOpened{screen}` | ルータ(M29) | 現在画面の写し更新 | 語彙外の画面 ID |

`dispatch` は `DispatchResult`(`stateChanged` / `changedPlacementCells` / `advanceContextRebuilt` / `advanceContextRestored`)を返す。UI の描画には使わず、**診断・計測・テスト用**である。
...
現状 engine 側に `apply(Command)` が無いため、**画面 → engine 純関数 → `stateApplied`** の形で繋ぐ。`src/engine/commands.ts` 実装後の置き換え手順は §7-2。
...
### 7-2. engine コマンドが実装されたときの置き換え

1. `store.ts` に `CommandAppliedEvent{ type:"commandApplied", command }` を足す。
2. `applyEvent` に分岐を足し、`apply(state, command)`(engine 純関数)を呼んで `installWorldState` へ渡す。commandLog(ADR-002)を持つならここが唯一の追記点。
3. `StateAppliedEvent` と `applyStateApplied` を**削除**する(暫定口を残さない)。テストは `commandApplied` へ置換する。
4. 配置が変わるコマンドでは `DispatchResult.changedPlacementCells` が非空になり、`advanceContext` が自動で作り直される(手で呼ばない)。
...
| 繋ぐもの | 材料(実装済み) | 繋ぎ方 |
|---|---|---|
| セーブ | `platform/saveScheduler.ts`(4 トリガ)+ `persistence.ts` | `dispatch` の後に `scheduler.notifyCommand()` 相当を呼ぶ。ストアはセーブを知らない |
