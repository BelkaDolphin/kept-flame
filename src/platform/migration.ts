...
//
//        **[台帳v26 必-1] `savedAtMs` はこの版を上げない**(判断の記録)。
//        エンベロープへ「最後に書いた壁時計時刻」(`persistence.ts` §4)を
//        足したが、`SAVE_FORMAT_VERSION` は 1 のまま据え置いた。理由:
//          ① **この軸の版差は前後どちらの向きにもハード拒否になる**。
//             `verifySaveRecord` は「現行版と等しくない」だけで
//             `PersistenceError` を投げる(未来版は §1(d) により migrate も
//             されない)。つまり版を 2 へ上げると、新ビルドが書いたセーブは
//             旧ビルドで**まったく読めなくなる** = stable/beta を行き来した
//             プレイヤーの進行が丸ごと失われる(`src/main.tsx` は
//             `loadFailed` バナーを出して新規ゲームを始める)。
//          ② 一方 `savedAtMs` は **省略可・checksum の対象外・欠けても
//             「経過 0」= 台帳v26 必-1 以前と同一挙動**という三重に無害な
//             フィールドである。旧ビルドがこのセーブを読んでも余分なキーを
//             見ないだけで、黙って壊れることも往復で何かが失われることも
//             ない(旧ビルドが書き戻すと savedAtMs は消えるが、それは
//             「不在時間が分からない」= 旧挙動へ戻るだけである)。
//          ③ ADR-012 [2026-07-30追記] の線引きは「**旧ビルドで読んだとき
//             黙って壊れるか**」であり、②のとおり壊れない。版を上げる側の
//             前例(M16/M21/M22/M52/M28)は全て「黙って壊れる」に該当した
//             ものであって、本件は該当しない。
//          ④ **代償が釣り合わない**: 得られるのは「いつからこの省略可
//             フィールドがあるか」の記録だけで、失うのは①の後方互換である。
//             記録はこのコメントで足りる。
//        逆に言えば、将来 `savedAtMs` を**必須**にする(無いセーブを拒否する
//        / 値を信頼して engine の入力に使う)なら、そのときは版を上げること。
//
