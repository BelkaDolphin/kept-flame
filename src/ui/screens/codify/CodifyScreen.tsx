...
// ===========================================================================
//   (a) **成文化の tick 結線が無い**。`scheduler.ts` の `PIPELINE_STAGE.codify`
//       は「成文化完了(未実装)」とコメントされており、`applyCodifyProgress`/
//       `completeCodification` を呼ぶ経路が scheduler/advance に無い(grep で
//       確認済み)。キュー投入(`beginCodification`)自体は実装済みで実際に
//       resource を消費するが、**投入後の進行度は 0 のまま動かない**。
//   (b) **キューの取消コマンドが無い**。`commands.ts` に cancel 系の型も予約も
//       無いため、本画面に「取消」ボタンは置かない。理由を明記して捏造しない
//       (FacilityScreen の増築コスト非表示と同じ作法)。
//
// ===========================================================================
  ...
  return (
    <section class="kf-codify-screen" aria-labelledby="kf-codify-screen-title">
      ...
      <p class="kf-codify-screen__note">
        現行の engine 実装では成文化の進行が tick に結線されていません。投入した記録は進行度 0
        のまま止まります(完了しません)。また、キューの取消コマンドも未実装です(投入は取り消せません)。
      </p>
