...
import { distanceBandLabel } from "../contentLabels";
...
export function memoirEntryText(entry: MemoirEntry): string {
  switch (entry.kind) {
    ...
    case "death":
      return "亡くなった。";
    case "bondMilestone":
      return `${entry.partnerId} との絆が深まった(節目${String(entry.tier)})。`;
    case "partnerLost":
      return `絆を結んでいた ${entry.partnerId} を喪った。`;
    case "explorationRescue":
      return `${distanceBandLabel(entry.band)}探索で ${entry.rescuedId} を保護した。`;
    case "bioCatchphrase":
...
export function MemoirRow({ feedEntry }: MemoirRowProps) {
  return (
    <li class="kf-chronicle__memoir-row">
      ...
      <span class="kf-chronicle__memoir-subject">{feedEntry.residentId}</span>
    ...
    <section class="kf-chronicle-screen" aria-labelledby="kf-chronicle-screen-title">
      ...
      {renderedLog.foldedCount > 0 && (
        <p class="kf-chronicle__folded">
          古い帰還記録 {renderedLog.foldedCount} 件は要約に畳まれています(上限50件・GDD
          8.4/12.5-9)。
        </p>
      )}

      <h3 class="kf-chronicle-screen__subtitle">住民 memoir(GDD 7.3)</h3>
      {memoirFeed.length === 0 ? (
