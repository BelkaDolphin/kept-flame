...
**[2026-07-27 整理]** summary.md 付録 A-6 の指摘(番号重複)を解消し、①②③の優先順で再構成。

1. **#12 再測定**(タイムスタンプ打刻 + 手順書だけ渡した別セッション)→ GDD 13.2「プロトタイプ計測後の全体スケジュール再計算」チェックポイントを数値で閉じる。#11 の一次データとしても現データより強い。
2. **②実機計測 = ユーザー実施**: `docs/measurements/device-testing-guide.md` + `dist/device/`(`npm run device:package` で再生成可)。実機入手後は **#10(ITP 7日待ち)を最初に開始**(待ち時間が最長)。裁定 D1〜D6 で判定前提は確定済み(450ms=512KB想定 / 1100ms=cold / `idbFirstTouchMs` は補助 / GCポーズはデスクトップ 0.83ms 代替 / ヒープは Android実測+iOS間接 / WebKit は実 Safari 非代替)。cold/warm 併記を忘れないこと。
3. **MVP実装ロードマップ化**(B1〜B9 裁定完了により**着手可能**): 残り 16.1〜17.5週の見積りをタスク分解。見積りは計画方式 ×3.5倍、予算単位はセッション(5h枠 ≒ 2M トークン)。
4. 教訓の運用化: 各タスク完了時に実消費を必ず MEMORY.md へ記録(先行フェーズは 16 中 7 タスクしか記録なし = #11 データが不完全になった)。
