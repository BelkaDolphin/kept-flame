...
4. **MVP実装着手**: ~~Phase 0(M1 W3/W4・M2)から~~ **[2026-07-28] Phase 0 は M45 を残して完了**(M44/M1/M2 済み)。**M45(ADR-030 レビュー分離)は bot 用 GitHub アカウント+PAT の作成がユーザー作業**のため保留 — 用意でき次第着手。~~次の実装タスクは Phase 1 の M3(セーブ層・依存M2✓)と M5(rules全系統化(1)・依存M2✓)で、並行投入可能。~~ **[2026-07-28] M3/M5完了**。~~M6/M4~~ **[2026-07-28完了]**。~~次の候補: M7/M8/M9~~ **[2026-07-29完了]**(Phase 1は M10 を残すのみ)。次の候補: **M10(conformance拡張#1、依存M2✓/M7✓/M9✓、Sonnet)** — M6持ち越し裁定①②+M7の★③trait ID統一+harnessData再生成+stat-経路5件とセットで。並行に**M11(住民寿命モデル、依存M7✓、Opus)**=クリティカルパス(M7→**M11**→M12→M13)。独立系の**M46(ADR-015 guardrail)**も投入可。~~M7/M8/M9の★判断3件はユーザー確認待ち~~ **[2026-07-29] 全件裁定済み・文書反映済み**(上記完了ログ参照)。M49(コマンド層)はM10/M11完了後の候補(M18/M29より前に必要)。
