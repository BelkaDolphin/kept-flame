...
- [2026-07-29] **M7/M8/M9完了**(3体並列Opus、Fable5が結合ツリーで全ゲート再実行=test 1294件/typecheck/lint/golden:check 37本差分なし/prettier合格を確認のうえ選択コミット。実消費 M7=200k/M8=~300k/M9=135k、合計~635k=見積り1.65Mの約39%):
  ...
  - **M7/M8/M9の要ユーザー判断(未処理)**: ①★combatPower重み配分0.35/0.30/0.20/0.15/0.00はGDD 8.2に無いengine定数→GDD追記要 ②★装備をcombatPowerに含めない解釈の確認 ③★記憶巧者ID=`traitLivingLibrary`採用(`traitMemoryKeeper`はsc09合成fixtureと衝突)→M10で統一推奨(base側をtraitMemoryKeeperに+sc09冪等化+balance.recallRiskParams.memoryKeeperTraitId張り) ④`@preact/signals`を入れるか(現状自前実装、差し替えはreactive.ts 1枚) ⑤`src/engine/commands.ts`の担当タスクがロードマップに無い(M8発見・stateApplied撤去の前提) ⑥M9の`unlocks[]`解釈(facility ID解決可能なら建設解禁)は施設14種content化の段で正式化要。
