...
4. **MVP実装着手**: ~~Phase 0(M1 W3/W4・M2)から~~ **[2026-07-28] Phase 0 は M45 を残して完了**(M44/M1/M2 済み)。**M45(ADR-030 レビュー分離)は bot 用 GitHub アカウント+PAT の作成がユーザー作業**のため保留 — 用意でき次第着手。~~次の実装タスクは Phase 1 の M3(セーブ層・依存M2✓)と M5(rules全系統化(1)・依存M2✓)で、並行投入可能。~~ **[2026-07-28] M3/M5完了**。~~M6/M4~~ **[2026-07-28完了]**。~~次の候補: M7/M8/M9~~ **[2026-07-29完了]**。~~M10/M11~~ **[2026-07-29完了]**(**Phase 1完了**・Phase 2はM11済)。**M11の★判断6件がユーザー確認待ち**(上記完了ログ。①②はGDD追記系=推奨どおりで軽い、④は数値バランス系でM39〜M41再評価前提の暫定承認でよい)。次の実装候補: **M12(memoirLog+bond、依存M11✓、Sonnet)**=クリティカルパス(M11→**M12**→M13)、並行に**M49(engineコマンド層、依存M7✓/M8✓、Opus)**と**M46(ADR-015 guardrail、独立、Sonnet)**。M16(footprint、依存M5✓)も投入可。M14(#5再計測)はM13後、M15(conformance拡張#2)はM10✓+M13後。
