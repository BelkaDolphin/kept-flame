...
4. **MVP実装着手**: ~~Phase 0(M1 W3/W4・M2)から~~ **[2026-07-28] Phase 0 は M45 を残して完了**(M44/M1/M2 済み)。**M45(ADR-030 レビュー分離)は bot 用 GitHub アカウント+PAT の作成がユーザー作業**のため保留 — 用意でき次第着手。~~次の実装タスクは Phase 1 の M3(セーブ層・依存M2✓)と M5(rules全系統化(1)・依存M2✓)で、並行投入可能。~~ **[2026-07-28] M3/M5完了**。~~M6/M4~~ **[2026-07-28完了]**。次の候補: **M7(住民ステータス/trait結線、依存M6✓、Opus)**が最優先(クリティカルパス M5→M6→**M7**→M11)、並行に**M8(状態管理+architecture.md、依存M5✓、Opus→Sonnet)**と**M9(静的グラフ解析、依存M6✓、Opus)**。~~M45~~ **[2026-07-28完全完了]**(PR #1実演済み・Phase 0全タスク完了)。M10(conformance拡張#1)はM7/M9完了後、持ち越し裁定①②とセットで。
