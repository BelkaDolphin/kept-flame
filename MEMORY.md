...
3. **②実機計測 = ユーザー実施**: `docs/measurements/device-testing-guide.md` + `dist/device/`(`npm run device:package` で再生成可)。実機入手後は **#10(ITP 7日待ち)を最初に開始**(待ち時間が最長)。**[2026-07-28更新] 手持ち実機 = iPhone/Mac系以外は揃っている** → Android/Surface で #1/#2/#8/#9b は今すぐ着手可。#10(ITP)と #7実iOS補完は iPhone 入手待ち(iOS依存のため代替不能)。裁定 D1〜D6 で判定前提は確定済み(450ms=512KB想定 / 1100ms=cold / `idbFirstTouchMs` は補助 / GCポーズはデスクトップ 0.83ms 代替 / ヒープは Android実測+iOS間接 / WebKit は実 Safari 非代替)。cold/warm 併記を忘れないこと。
