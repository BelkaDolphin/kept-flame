...
3. **②実機計測 = ユーザー実施**: `docs/measurements/device-testing-guide.md` + `dist/device/`(`npm run device:package` で再生成可)。実機入手後は **#10(ITP 7日待ち)を最初に開始**(待ち時間が最長)。裁定 D1〜D6 で判定前提は確定済み(450ms=512KB想定 / 1100ms=cold / `idbFirstTouchMs` は補助 / GCポーズはデスクトップ 0.83ms 代替 / ヒープは Android実測+iOS間接 / WebKit は実 Safari 非代替)。cold/warm 併記を忘れないこと。
4. **MVP実装着手**: ~~Phase 0(M1 W3/W4・M2)から~~ **[2026-07-28] Phase 0 は M45 を残して完了**(M44/M1/M2 済み)。**M45(ADR-030 レビュー分離)は bot 用 GitHub アカウント+PAT の作成がユーザー作業**のため保留 — 用意でき次第着手。次の実装タスクは Phase 1 の M3(セーブ層・依存M2✓)と M5(rules全系統化(1)・依存M2✓)で、並行投入可能。
...
- ~~ユーザー裁定待ち: summary.md §6.1 の B1〜B9~~ **[2026-07-27完了]** B1〜B9 / D1〜D7 / N1〜N13 を全件「推奨どおり」で裁定・文書反映済み(下記完了ログ参照)。N14(失効PAT削除)のみユーザー自身の GitHub 操作待ちで未処理。
