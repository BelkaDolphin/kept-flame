...
1. **未pushコミットのpush = ユーザー作業**: `git push origin main`(0c390f3 以降、裁定反映/W1・W2/#12再測定/ロードマップ/総括更新の5件。統率側の push は auto mode 分類器に拒否されるためユーザー実行が必要)。
2. **ロードマップ §6.2「未計上作業」の裁定 = ユーザー判断**: ADR-031(stable/beta 前倒し=ADR自身がMVP対象と明記するのに工数行が無い・実質的な抜けの疑い)、ADR-015/021/022/025/030、オーサリングツール本体。タスク化するか MVP 外へ明示的に出すかを決める。
