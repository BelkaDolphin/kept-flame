...
4. **MVP実装着手**: ~~Phase 0(M1 W3/W4・M2)から~~ **[2026-07-28] Phase 0 は M45 を残して完了**(M44/M1/M2 済み)。**M45(ADR-030 レビュー分離)は bot 用 GitHub アカウント+PAT の作成がユーザー作業**のため保留 — 用意でき次第着手。~~次の実装タスクは Phase 1 の M3(セーブ層・依存M2✓)と M5(rules全系統化(1)・依存M2✓)で、並行投入可能。~~ **[2026-07-28] M3/M5完了**。次の候補: **M6(研究・成文化・記録媒体2種、依存M5✓、Opus)**が最優先(クリティカルパス)、並行に**M4(export/import、依存M3✓、Sonnet)**と**M8(状態管理+architecture.md、依存M5✓、Opus→Sonnet)**。M6着手時に★裁定4件(施設14種content+既存2テスト改変許可 / traitクランプ / yieldMul予約語 / 廃材交換比)のユーザー確認を先に取ること。
